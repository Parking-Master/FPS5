...
user = {
  ...
  load: function(callback = () => {}) {
    ...
    if (sessionKey) {
      ...
      let userData = localStorage["local-user-data"];
      if (!userData) {
        function generateName() {
          ...
          function getRandomInt(min, max) {
        	...
        	return (name.replace(/ /g, "") + Math.random().toString().substr(Math.random().toString().indexOf(".") + 1).substr(0, 4));
        }
        localStorage.setItem("local-user-data", JSON.stringify({
